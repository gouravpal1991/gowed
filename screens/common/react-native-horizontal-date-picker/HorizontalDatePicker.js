...
export default class HorizontalDatePicker extends Component {
  ...
  componentDidMount = () => {
    ...
    if (minDate && maxDate) {
      if (pickerType === 'date' || pickerType == 'datetime')
        dateArray = this.getDateList(minDate, maxDate);
      if (pickerType === 'time' || pickerType == 'datetime') {
        ...
        if (
          ...
          sameDay = true;
        timeArray = this.getTimeList(
          minDate,
          sameDay ? maxDate : moment(minDate).set({hours: 24, minutes: 0}),
        );
      }
    } else if (minDate) {
      if (pickerType === 'date' || pickerType == 'datetime') {
      ...
      if (pickerType === 'time' || pickerType == 'datetime') {
        timeArray = this.getTimeList(
          minDate,
          moment(minDate).set({hours: 24, minutes: 0}),
        );
      }
    } else if (maxDate) {
      if (pickerType === 'date' || pickerType == 'datetime') {
        dateArray = this.getDateList(
    ...
    const newDateArray = dateArray.map((item) => ({
      ...
      isSelected: false,
      weekDayName: moment(item, defaultFormatDate).format(weekDayNameFormat),
    }));
    ...
    if ((pickerType === 'time' || pickerType == 'datetime') && onTimeSelected) {
      if (defaultSelected && isCurrentFoundTime) {
        ...
        onTimeSelected(
          moment(newTimeArray[0].time, defaultFormatTime).format(
            returnTimeFormat,
          ),
        );
      }
        ...
        datetime:
          pickerType == 'datetime'
            ? defaultSelected && isCurrentFoundDate && isCurrentFoundTime
              ...
              : moment(
                  newDateArray[0].date + newTimeArray[0].time,
                  defaultFormatDate + defaultFormatTime,
                ).format(returnDateTimeFormat)
            : '',
      });
    }
    this.setState({
  ...
  getTimeList = (minDate, maxDate) => {
    ...
    let currentDate =
      tmpCurrentDate.minute() ||
      tmpCurrentDate.second() ||
      tmpCurrentDate.millisecond()
        ? tmpCurrentDate.add(1, 'hour').startOf('hour')
        : tmpCurrentDate.startOf('hour');
    const stopDate = moment(maxDate);
    while (currentDate <= stopDate) {
      if (
        timeArray.indexOf(moment(currentDate).format(defaultFormatTime)) === -1
      ) {
  ...
  changeTimeArrayOnDateChange = (date) => {
    ...
    if (minDate && maxDate) {
      ...
      timeArray = this.getTimeList(
        moment(maxDate).set({hours: 0, minutes: 0}),
        maxDate,
      );
    } else if (
      date === arrayDates[0].date &&
      date === moment().format(defaultFormatDate)
    ) {
      timeArray = this.getTimeList(
        moment(),
        moment().set({hours: 24, minutes: 0}),
    ...
    if (onDateTimeSelected) {
      ...
      this.setState({
        arrayTimes: newTimeArray,
        selectedMonth: moment(date, defaultFormatDate).format('MMM'),
      });
    }
  };
  ...
  onVisibleItemChange = ({viewableItems}) => {
    ...
    if (yearSelected) {
      if (
        moment(yearSelected, yearFormat) !==
        moment(viewableItems[0].item.year, yearFormat)
      ) {
        this.setState({
          yearSelected: moment(viewableItems[0].item.year, yearFormat).format(
            yearFormat,
          ),
        });
      }
  ...
  onPressDate = (item) => () => {
    ...
    this.setState({
      arrayDates,
      selectedMonth: moment(item.date, defaultFormatDate).format('MMM'),
    });
    if (onDateSelected)
      onDateSelected(
  ...
  onPressTime = (item) => () => {
    ...
    if (onDateTimeSelected) {
      onDateTimeSelected({
        date:
          pickerType === 'date' || pickerType == 'datetime'
            ? moment(dateSelected[0].date, defaultFormatDate).format(
                returnDateFormat,
              )
            : '',
        time:
  ...
  renderDateItem = ({item}) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={[styles.itemViewStyle]}
        onPress={this.onPressDate(item)}>
        <Text style={{paddingVertical: 15, fontFamily: 'Roboto-Regular'}}>
          {item.weekDayName}
        </Text>
        <View
          style={
        ...
        {(pickerType === 'date' || pickerType === 'datetime') && isShowYear && (
          <View style={[styles.yearTextContainer, yearContainerStyle]}>
            <Text style={[styles.yearTextStyle, yearTextStyle]}>
              {this.state.selectedMonth + ' ' + yearSelected}
            </Text>
          </View>
        )}
        {(pickerType === 'date' || pickerType === 'datetime') && (
          <ImageBackground
            style={[styles.datePickerContainer, datePickerContainerStyle]}
            source={datePickerBG || null}>
            <FlatList
              horizontal
...
HorizontalDatePicker.propTypes = {
  ...
  yearContainerStyle: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.object,
    PropTypes.array,
  ]),
  yearTextStyle: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.object,
    PropTypes.array,
  ]),
  ...
  unSelectedTextStyle: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.object,
    PropTypes.array,
  ]),
  datePickerContainerStyle: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.object,
    PropTypes.array,
  ]),
  timePickerContainerStyle: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.object,
    PropTypes.array,
  ]),
